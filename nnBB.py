...
# Get the absolute path of the parent directory (food)
parent_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))

# Add it to sys.path
sys.path.insert(0, parent_dir)

# Now you can import mymodule
import inference_naive_bayes as nb 

# --- Data Preparation with Naive Bayes Features ---
def augment_with_nb_features(csv_path, mean=None, std=None, is_train=True):
    """Load CSV, generate Naive Bayes features, and augment data"""
    # Read CSV and create feature arrays
    df = pd.read_csv(csv_path)
    
    
    # Create numpy arrays from all columns (excluding NaNs)
    feature_arrays = df.apply(
        lambda row: np.array([v for v in row.values if pd.notnull(v)]), 
        axis=1
    ).tolist()
    
    # Generate Naive Bayes logits
    nb_features = []
    for array in feature_arrays:
        _, logits = nb.predict_smart(array, verbose=False)
        logit_values = [logits["Pizza"], logits["Shawarma"], logits["Sushi"]]
        # Apply sigmoid to normalize between 0-1
        normalized_logits = np.array(logit_values) / np.sum(logit_values)
        nb_features.append(normalized_logits)
    nb_features = np.array(nb_features)
    
    # Load and standardize original features
    data_type = 'train' if is_train else 'test'
    X_original = np.load(f'X_clean_{data_type}.npy')
    ...
    # Drop the first, fourth, sixth, seventh, eighth, and ninth columns
    columns_to_drop = [0, 3, 5, 6, 7, 8]
    X_std = np.delete(X_std, columns_to_drop, axis=1)
    # Concatenate features
    X_augmented = np.hstack((X_std, nb_features))
    return X_augmented, mean, std
...
class NeuralNetwork:
    def __init__(self, input_size, hidden_sizes, output_size, alpha=0.005, l2_lambda=0.05,
                 clip_threshold=1.0):
        self.clip_threshold = clip_threshold  # Weight clipping threshold
    ...
    def backward(self, X, y):
        ...
        if self.clip_threshold is not None:
            self.W1 = np.clip(self.W1, -self.clip_threshold, self.clip_threshold)
            self.W2 = np.clip(self.W2, -self.clip_threshold, self.clip_threshold)
            self.W3 = np.clip(self.W3, -self.clip_threshold, self.clip_threshold)
    ...
    def train(self, X_train, y_train, X_val, y_val, epochs=500, batch_size=32, patience=10):
        ...
        train_acc = np.mean(train_preds == np.argmax(t_train_one_hot, axis=1))
...
# Load augmented validation data
X_val_augmented, _, _ = augment_with_nb_features('food_test.csv', mean=mean, std=std, is_train=False)

# --- Neural Network Code (Modified Input Size) ---
# Load labels and convert to one-hot
t_train = np.load('t_clean_train.npy')
t_valid = np.load('t_clean_test.npy')
num_classes = 3
t_train_one_hot = np.eye(num_classes)[t_train]
t_valid_one_hot = np.eye(num_classes)[t_valid]

# Update input size to include NB features
input_size = X_train_augmented.shape[1]
hidden_sizes = (128, 64)
output_size = num_classes

# Initialize network with new input size
nn = NeuralNetwork(
    ...
    alpha=0.05,
    l2_lambda=0.25,
    clip_threshold=4  # Uncomment to use weight clipping
)
...
train_acc = np.mean(train_preds == np.argmax(t_train_one_hot, axis=1))
valid_acc = np.mean(valid_preds == np.argmax(t_valid_one_hot, axis=1))
...
# # Example test
# X_test_sample = valid_preds[:5]
# print("Predictions:", nn.predict(X_test_sample))